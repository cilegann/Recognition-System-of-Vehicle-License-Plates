function [result,plate,rs,cs,h,w]=locate(input)
%% prepare data
origimg=input;
...
for i=1:height
  for j=1:(width/3)
       check=(img(i,j,1)>r_topmark)+(img(i,j,2)>g_topmark)+(img(i,j,3)>b_topmark);
      if (img(i,j,1)>r_topmark)&&(img(i,j,2)>g_topmark) || (img(i,j,3)>b_topmark)&&(img(i,j,2)>g_topmark) || (img(i,j,1)>r_topmark)&&(img(i,j,3)>b_topmark)
          bw(i,j)=1;
      end
  end
end
